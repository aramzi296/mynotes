            ...
            <div class="mb-8 rounded-[32px] border border-[#E8E6E1] bg-white p-8 shadow-sm dark:border-[#3E3E3A] dark:bg-[#111111]">
                ...
                <form id="note-form" action="{{ route('notes.update', $note) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input id="note-content" type="hidden" name="note" value="{{ old('note', $note->content) }}" />
                        <div id="quill-editor" class="min-h-[280px] rounded-[24px] border border-[#e3e3e0] bg-[#FCFBF8] p-4 text-sm leading-7 text-[#1b1b18] shadow-sm outline-none transition dark:border-[#3E3E3A] dark:bg-[#121212] dark:text-[#F7F7F5]">{!! old('note', $note->content) !!}</div>
                    </div>
